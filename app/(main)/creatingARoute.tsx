...
const CreatingARoute = () => {
  ...
  useEffect(() => {
      ...
      const init = async () => {
        ...
        // Получаем текущие координаты сразу
        ...
        // После этого — включаем постоянный трекинг
        startTracking();
        setTime(0);
        setDistance(0);
      };
  ...
  const handleFinish = () => {
    if (locations && locations.length >= 2 && distance > 0) {
...
//   >([]);
//   const mapRef = useRef<MapView | null>(null);
//   // для корректного приближения к текущей позиции
...
//
//   // приближение при первом открытии карты
//   const handleMapReady = () => {
//     setIsMapReady(true);
//
//     // если уже есть координаты — сразу приближаем
//     if (locations.length > 0 && mapRef.current) {
//       const loc = locations[locations.length - 1];
//       mapRef.current.animateToRegion(
//         {
